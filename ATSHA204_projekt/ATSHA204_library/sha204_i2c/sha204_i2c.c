...
#include "../sha204_return_codes/sha204_return_codes.h"
#include "../sha204_timer/sha204_timer.h"
#include <util/delay.h>

#define SHA204_I2C_DEFAULT_ADDRESS ((uint8_t) 0xC8)

enum i2c_word_address {
	SHA204_I2C_PACKET_FUNCTION_RESET = (uint8_t) 0x00,  // reset device
	SHA204_I2C_PACKET_FUNCTION_SLEEP = (uint8_t) 0x01,  // put device into Sleep mode
	SHA204_I2C_PACKET_FUNCTION_IDLE = (uint8_t) 0x02,   // put device into Idle mode
	SHA204_I2C_PACKET_FUNCTION_NORMAL = (uint8_t) 0x03 // write / evaluate data that follow this word address byte
};
...
uint8_t sha204p_wakeup(void)
{
	...
	_delay_ms(SHA204_WAKEUP_DELAY); // DOD?LAT DELAY
...
uint8_t sha204p_send_slave_address(uint8_t read)
{
	...
	uint8_t ret_code = i2c_send_start(); // funguje, ret_code = 0 success
	if (ret_code != I2C_FUNCTION_RETCODE_SUCCESS)
		return ret_code;

	ret_code = i2c_send_bytes(1, &sla); // nefunguje, ret_code = 32 nack
