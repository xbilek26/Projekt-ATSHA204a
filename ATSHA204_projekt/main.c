...
int main(void) {
	...
	if (read_config_status == SHA204_SUCCESS) {
		...
		printf("Error reading config zone! Return code: %d", read_config_status);
	}
	...
	if (write_data_status == SHA204_SUCCESS) {
		...
		printf("Error writing to data zone! Return code: %d\n\r", write_data_status);
	}
	
	// ********************* READ DATA *********************
	
	uint8_t read_tx_buffer[34];
    ...
    if (read_data_status == SHA204_SUCCESS) {
        for (int i = 0; i < READ_32_RSP_SIZE; i++)
            printf("Byte %d: 0x%02X\n\r", i, read_rx_buffer[i]);
    } else {
        printf("Error reading data zone! Return code: %d\n\r", read_data_status);
    }
	
	// ********************* CALCULATE HASH DIGEST *********************
	
	uint8_t message[] = "Hello, world!";
    int32_t len = sizeof(message) - 1;

    uint8_t digest[32];

    sha204h_calculate_sha256(len, message, digest);
	
	printf("Message: '%s'\n\r", message);
	
    printf("SHA256 Digest of message:\n\r");
    for (int i = 0; i < 32; i++) {
        printf("%02x", digest[i]);
    }
    printf("\n\r");
